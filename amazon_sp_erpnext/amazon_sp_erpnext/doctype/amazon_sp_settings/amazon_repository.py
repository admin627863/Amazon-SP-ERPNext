...
import amazon_sp_erpnext.amazon_sp_erpnext.doctype.amazon_sp_settings.amazon_sp_api as sp_api


class AmazonRepository:
    def __init__(self, amz_setting_name) -> None:
        self.amz_setting = frappe.get_doc("Amazon SP Settings", amz_setting_name)
    ...
    def create_sales_order(self, order):
        ...
        if sales_order:
            ...
            delivery_date = dateutil.parser.parse(order.get("LatestShipDate")).strftime(
                "%Y-%m-%d"
            )
    ...
    def create_item(self, amazon_item, asin, sku):
        if frappe.db.get_value(
            "Ecommerce Item", filters={"integration_item_code": asin}
        ):
            return asin
        ...
        if frappe.db.exists("Item", asin):
            ...
            item.image = (
                amazon_item.get("AttributeSets")[0].get("SmallImage", {}).get("URL")
            )
        ...
        if report_id:
            ...
            if report_document:
                ...
                for item in report_document:
                    ...
                    amazon_item = catalog_items.get_catalog_item(asin=asin).get(
                        "payload"
                    )
