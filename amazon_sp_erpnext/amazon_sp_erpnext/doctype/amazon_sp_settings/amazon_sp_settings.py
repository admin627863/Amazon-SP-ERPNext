...
class AmazonSPSettings(Document):
    def validate(self):
        if self.is_active == 1:
        ...
        if self.max_retry_limit and self.max_retry_limit > 5:
            frappe.throw(
                frappe._(
                    "Value for <b>Max Retry Limit</b> must be less than or equal to 5."
                )
            )
...
def schedule_get_order_details():
    ...
    for amz_setting in amz_settings:
        after_date = frappe.get_value("Amazon SP Settings", amz_setting, "after_date")
...
def migrate_old_data():
    ...
    if column_exists:
        ...
        items = (
            frappe.qb.from_(item).select("*").where(item.amazon_item_code.notnull())
        ).run(as_dict=True)
